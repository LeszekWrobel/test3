...
{	// obliczamy i ustawiamy pozostałe zmienne
	$id = $_SESSION['id'];
	if ((isset($_SESSION['number_of_teeth']))&&($_SESSION['number_of_teeth']!='')&&(isset($_SESSION['reps']))&&($_SESSION['reps']!='')&&(isset($_SESSION['quantity_er']))&&($_SESSION['quantity_er']!='')&&(isset($_SESSION['ilosc_uzytkow']))&&($_SESSION['ilosc_uzytkow']!='')&&($_SESSION['ilosc_uzytkow']!=0))
 ...
 if ($_SESSION['mode']==='edit')
		{
			...
			$_SESSION['date_of_insertion'] = date('Y-m-d'); //date dodania ustawiamy na aktualną date('Y-m-d') w trybie wprowadzania nowej karty;
		}
	$_SESSION['id_autora'] ; // autora wpisujemy w to samo pole niezaleznie czy tworzu kartę czy tylko ja edytuje
...
echo 'Pola obowiązkowe :<br>Kod karty produkcji, Klient, Wzór ';
